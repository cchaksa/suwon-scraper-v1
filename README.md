...
- `SCRAPE_RESULT_BUCKET` (필수, 예: `develop-shadow-scraper-results`)
- `SCRAPE_RESULT_PREFIX=scrape-results/`
- `SCRAPE_RESULT_STORAGE_CLASS=STANDARD`
- `SCRAPE_RESULT_KMS_KEY_ARN` (선택, 미지정 시 AES256)
- `SCRAPE_RESULT_RETENTION_DAYS` (선택, 메타데이터 기록용)
- ECS Task Role에는 최소한 `s3:PutObject`, `s3:GetObject`, `s3:AbortMultipartUpload`, `s3:ListBucket` 권한을 `SCRAPE_RESULT_BUCKET` prefix 범위로 허용해야 한다.
